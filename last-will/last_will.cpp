...
namespace estate_executor
{
	...
	{
		int zhangNumberPart = zhang::bank_number_part(secret_modifier);
		int khanNumberPart = khan::bank_number_part(secret_modifier);
		int garciaNumberPart = garcia::bank_number_part(secret_modifier);

		return zhangNumberPart + khanNumberPart + garciaNumberPart;
	}

	int assemble_code()
	{
		int redCodeFrag = zhang::red::code_fragment() + khan::red::code_fragment() + garcia::red::code_fragment();
		int blueCodeFrag = zhang::blue::code_fragment() + khan::blue::code_fragment() + garcia::blue::code_fragment();

		return redCodeFrag * blueCodeFrag;
	}
}